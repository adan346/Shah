...
module.exports.handleEvent = function ({ api, event }) {
  ...
  if (event.body.toLowerCase() == "by" || event.body.toLowerCase() == "bye") {
    return api.sendMessage({ sticker: "629261957190121" }, event.threadID);
  }
  ...
  if (
    event.body.toLowerCase() == "tharki bot" ||
    event.body.toLowerCase() == "Bot tharki"
  ) {
    return api.sendMessage({ sticker: "526225161444249" }, event.threadID);
  }

  if (
    event.body.toLowerCase() == "lanti bot" ||
    event.body.toLowerCase() == "lanti hai"
  ) {
    return api.sendMessage({ sticker: "466044071430389" }, event.threadID);
  }

  if (
    event.body.toLowerCase() == "assalamualaikum" ||
    event.body.toLowerCase() == "Assalamualaikum"
  ) {
  ...
  if (
    event.body.toLowerCase() == "ma ja rha" ||
    event.body.toLowerCase() == "ma ja rhi"
  ) {
    return api.sendMessage({ sticker: "466044418097021" }, event.threadID);
  }

  if (
    event.body.toLowerCase() == "nikal" ||
    event.body.toLowerCase() == "nikl"
  ) {
    return api.sendMessage({ sticker: "466044418097021" }, event.threadID);
  }
